...
class Usuario
{
    ...
    public static function todos_usuarios()
    {
    ...
    public function uptade_user()
    {
        //$_POST valores de la actualizacion
...
class Mascota
{
    ...
    public function uptade_pet()
    {
        //$_POST valores de la actualizacion ofcourse
    ...
    {
        //$_POST valores para agregar ofcourse
        $revisar = getimagesize($_FILES["imagen"]["tmp_name"]); //se toma tamaño de la imagen
        if ($revisar !== false) {
            //y se verifica si tiene  tamaño para validar si se cargo o no
    ...
    public function cambiar_reserva()
    {
        //$_POST valores de la actualizacion ofcourse
...
class Fundacion
{
    ...
    public static function fundaciones()
    {
        //Retorna todas las fundaciones que existen
        $db        = Conectar::conexion();
        $resultado = mysqli_query($db, "SElECT * FROM fundacion") or die("Algo ha ido mal en la consulta a la base de datos");
        return $resultado;
    }

    public static function donacion()
    {
        //Crea  una donacion para una fundacion
        $user_id   = $_SESSION['user']['id'];
